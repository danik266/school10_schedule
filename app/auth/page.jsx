...
export default function AuthPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        // Вот здесь мы проверяем роль, которая пришла с бэкенда
        if (data.role === "admin") {
          router.push("/admin"); // Кидаем админа в его панель
        } else {
          router.push("/schedule-view"); // Обычных юзеров кидаем в расписание
        }
      } else {
        setError(t[lang].err);
      }
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex flex-1 flex-col md:flex-row justify-center items-center py-10 gap-50 px-6">
        ...
        <div className="rounded-xl p-8 w-full max-w-md text-center">
          ...
          >
            <h2 className="text-3xl font-bold text-center text-[#0d254c] mb-4">
              {t[lang].btn}
            </h2>
